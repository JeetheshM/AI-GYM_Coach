...
class VideoProcessorClass(VideoProcessorBase):
    ...
    def _draw_skeleton(self, img, landmarks):
        ...
        # 1. Draw glowing neon green bones (dual-layer glow effect)
        for start_idx, end_idx in POSE_CONNECTIONS:
            ...
            if p1.visibility > 0.6 and p2.visibility > 0.6:
                ...
                # Outer glow line (thicker dark neon green)
                cv2.line(img, pt1, pt2, (0, 180, 0), 12, cv2.LINE_AA)
                # Inner core line (electric bright lime green)
                cv2.line(img, pt1, pt2, (50, 255, 50), 5, cv2.LINE_AA)
        
        # 2. Draw glowing neon green joint nodes (multi-halo dots matching the design)
        for lm in landmarks:
            if lm.visibility > 0.6:
                center = (int(lm.x * w), int(lm.y * h))
                # Outer glow halo
                cv2.circle(img, center, 14, (0, 220, 50), -1, cv2.LINE_AA)
                # Middle bright neon ring
                cv2.circle(img, center, 9, (100, 255, 100), -1, cv2.LINE_AA)
